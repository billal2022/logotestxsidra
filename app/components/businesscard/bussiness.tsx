  ...
  const setState = (action: T | ((prevState: T) => T)) => {
    const newState =
      typeof action === "function"
        ? (action as Function)(history[index])
        : action;
...
interface BusinessCardContextType {
  ...
  setState: (
    action:
      | BusinessCardState
      | ((prevState: BusinessCardState) => BusinessCardState)
  ) => void;
  undo: () => void;
  redo: () => void;
...
const BusinessCardProviderComponent: React.FC<{
...
const ModularBusinessCardStudio: React.FC<ModularBusinessCardStudioProps> = ({
  mode,
  colorPattern,
  LogoDisplay,
}) => {
  ...
  useEffect(() => {
    const selectedSvgTemplate = svgTemplates.find(
      (template) => template.id === selectedTemplate
    );
    if (selectedSvgTemplate) {
      const newBackground = generateCreativeProfessionalBackground(
        colorPattern,
        selectedSvgTemplate
      );
      setState((prevState) => {
        const updateElementsWithNewColors = (elements: CardElement[]) => {
          ...
          const outlineColor = isLightColor(bgColor)
            ? "rgb(255, 255, 255)"
            : "rgb(0, 0, 0)";
        ...
        const frontElements =
          prevState.front.elements.length > 0
            ? updateElementsWithNewColors(prevState.front.elements)
            : createDefaultCardElements(
                colorPattern,
                "front",
                prevState.language
              );
        ...
        const backElements =
          prevState.back.elements.length > 0
        ...
        return {
          ...
          front: {
            ...
            backgroundColor: `rgb(${Math.round(
              colorPattern.colors[0].r * 255
            )}, ${Math.round(colorPattern.colors[0].g * 255)}, ${Math.round(
              colorPattern.colors[0].b * 255
            )})`,
          },
          back: {
            ...prevState.back,
  ...
  const createDefaultCardElements = (
    colorPattern: ColorPattern,
    side: "front" | "back",
    lang: "english" | "arabic"
  ): CardElement[] => {
    ...
    const determineColors = (): { textColor: string; outlineColor: string } => {
      if (lightness > 0.8) {
        ...
        const textHsl = `hsl(${complementaryHue * 360}, 100%, ${
          lightness > 0.5 ? 0 : 100
        }%)`;
        const outlineHsl = `hsla(${complementaryHue * 360}, 100%, ${
          lightness > 0.5 ? 100 : 0
        }%, 0.5)`;
    ...
    const createTextElement = (
      ...
      props: {
        ...
        letterSpacing: 0.5,
        opacity: 1,
        textDecoration: `4px 0 0 ${outlineColor}`, //  effect
      },
    });
    ...
    if (side === "front") {
      ...
      if (lang === "english") {
        return [
          ...
          createTextElement(
            "We specialize in financial management and consulting.",
            5,
            30,
            10
          ),
        ...
        return [
          ...
          createTextElement(
            ".نحن متخصصون في الإدارة المالية والاستشارات",
            23.5,
            30,
            12
          ),
  ...
  const toggleLanguage = () => {
    setState((prevState) => ({
      ...
      front: {
        ...prevState.front,
        elements: createDefaultCardElements(
          colorPattern,
          "front",
          prevState.language === "english" ? "arabic" : "english"
        ),
      },
      back: {
        ...prevState.back,
  ...
  const updateElement = useCallback(
    (id: string, props: Partial<CardElement["props"]>) => {
      setState((prevState) => ({
        ...prevState,
        [activeSide]: {
          ...prevState[activeSide],
          elements: prevState[activeSide].elements.map((el) =>
  ...
  const handleInteractionStart = (
    ...
    if (element) {
      ...
      if (cardRect) {
        ...
        const offsetX =
          clientX - (cardRect.left + (element.props.x / 100) * cardRect.width);
        const offsetY =
          clientY - (cardRect.top + (element.props.y / 100) * cardRect.height);
    ...
    (e: MouseEvent | TouchEvent) => {
      if (dragging && selectedElement) {
        ...
        if (cardRect) {
          ...
          const x =
            ((clientX - cardRect.left - dragOffset.x) / cardRect.width) * 100;
          const y =
            ((clientY - cardRect.top - dragOffset.y) / cardRect.height) * 100;
  ...
  const exportToImage = async () => {
    ...
    if (node) {
      ...
      try {
        ...
        if (
          state[activeSide].backgroundImage &&
          state[activeSide].backgroundImage.trim().startsWith("<svg")
        ) {
